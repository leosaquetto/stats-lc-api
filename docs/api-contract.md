...
| `/api/replay` | `userId=<user id>` or `user=<user>`, optional `period=today|week|month|year|all` default `today`, `force=1` | Single payload for the Replay section. | `period`, `totalSongs`, `totalDurationMs`, `durationMs`, `minutes`, `hours`, `topArtists` top 20, `topTracks` top 30, and `topAlbums` top 15. Top-list failures are isolated under optional `errors`; stats failure fails the request. |
