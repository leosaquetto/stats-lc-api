...
- Current production rollup: `33abac4`, `f34ed76`, `cfcb20d`, and `1f4c2cf`.
...
- The API test suite passed 61 tests after deadline, partial-response, stale, timing, and top-empty-range coverage was added.
- These measurements are snapshots, not SLOs. Compare future runs using the same region, cache state, and query.
...
| `/api/track-story` | `user=<user>`, `track=<track id>`, optional `album=<album id>`, `artists=<csv artist ids>`, `releaseDate=<date or epoch ms>` | Progressive Bottom Track Stats story payload for one user/track. | `counts`, `history.bestYear.{year,count,previousYearCount,nextYearCount}`, advanced metrics for tracks with more than 10 plays, `social`, `specialCards`, and truthful `coverage.partial`. It never uses `force=1`; slow proof work returns partial coverage instead of failing the modal. |
