...
| `/api/lyrics` | `title=<track title>`, optional `artist=<artist name>` | Genius lyrics availability match. | Uses the server-side `GENIUS_ACCESS_TOKEN` to search Genius and returns `hasLyrics` plus `match.{title,artist,url,confidence}`. It does not expose lyrics text or the Genius token. |
